...
# CNF model
model = MLP(dim=dim, w=256, time_varying=True)
...
optimizer = torch.optim.Adam(cnf_model.parameters(), lr=1e-3)
...
n_samples = 21
tspan=torch.linspace(1, 0, n_steps)
...
base_samples = base.sample((n_samples,))[:,None]
with torch.no_grad():
            ...
            if t > 0:
                aug_traj = (
                    cnf_model[1].trajectory(
                        ...
                        t_span=torch.linspace(0, t, 201)))[-1].cpu()
...
def plot_trajectories(trajectories, lps, ax):
    probs = lps.exp()
    vmax = base.log_prob(torch.tensor(1.0)).exp()
...
plt.savefig("figures/cnf_aaai.png", 
